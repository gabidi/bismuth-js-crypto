...
module.exports = ({} = {}) => {
  ...
  const util = (Crypto.util = {
    ...
    bytesToWords: function (bytes) {
      for (let words = [], i = 0, b = 0; i < bytes.length; i++, b += 8) { words[b >>> 5] |= (bytes[i] & 0xff) << (24 - (b % 32)) }
      return words
    },

    // Convert big-endian 32-bit words to a byte array
    wordsToBytes: function (words) {
      for (let bytes = [], b = 0; b < words.length * 32; b += 8) { bytes.push((words[b >>> 5] >>> (24 - (b % 32))) & 0xff) }
      return bytes
    },
